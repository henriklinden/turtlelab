...
import errno
import inspect
from mvNeuro.data import basic as mvData_basic
...
class Spike_sorting:    
    ...
    def make_KWIK_files(self):
        #make sorting directory
        folder_path = os.path.join(self.rec.par['root'],'sorting_' + self.name)
        try:
        ...
        path_default = os.path.dirname(os.path.abspath(filename))
        execfile(path_default + '/defaultparameters.py', {}, parameters)
        #overload parameters
        tmp = self.rec.par['sr_ampli'].values()
        parameters['sample_rate'] = sum(tmp)/len(tmp)
        #make raw_data_files list
        raw_data_files = []
        for data_nb in self.rec.par['labbook']:
            ampli_nb = self.rec.par['labbook'][data_nb][1]
            filepath_ampli = os.path.join(self.rec.par['root_raw'],
	        self.rec.par['folder_ampli'],
		self.rec.par['base_ampli'] + ampli_nb + '.dat')
            raw_data_files.append(filepath_ampli)
        parameters['raw_data_files'] = raw_data_files
        parameters['experiment_name'] = self.name
        parameters['prb_file'] = 'probe.prb'
        parameters['nchannels'] = self.rec.par['nb_channels']
        #write parameter file
        mvData_basic.write_vars_to_file(fid,parameters)
        #write probe file
        filepath = os.path.join(
                self.rec.par['root'],
                'sorting_' + self.name,
        ...
        mvData_basic.write_vars_to_file(fid, {'channel_groups':probes})
    ...
    def save_klusta_files(self):
        ...
        fid = open(filepath,'w')
        probes = self.generate_probes()
        self.write_vars_to_file(fid, {'channel_groups':probes})
    
    
        
    def generate_probes_old(self):
        """One probe for each shank"""
        probes = {}
    ...
    def generate_probes(self):
        """One probe for each electrode"""
        channel_groups = {}
        for probe_nb, probe_info in self.probe_list.iteritems():
            if isinstance(probe_info, int):
                tmp = self.generate_probe(probe_info)
            else:
                tmp = self.generate_probe(probe_info[0], probe_info[1])
            group = {}
            group['channels'] = []
            group['graph'] = []
            group['geometry'] = {}
            for shank_nb, shank in tmp.items():
                group['channels'].extend(shank['channels'])
                group['graph'].extend(shank['graph'])
                group['geometry'] = dict(group['geometry'].items() + shank['geometry'].items())
            channel_groups[probe_nb] = group
        return channel_groups
        
    def generate_probe(self, ch_offset, reverse=False):
        if reverse:
            #use if adapter from electrode to omnitronics is reversed
            probe = {}
            probe[0] = {'channels':[63,50,59,61,58,53,55,54]}
            probe[1] = {'channels':[57,5,48,49,52,3,1,51]}
            probe[2] = {'channels':[0,14,7,6,2,12,9,4]}
            probe[3] = {'channels':[8,56,10,62,15,60,13,11]}
            probe[4] = {'channels':[38,22,32,20,34,17,21,19]}
            probe[5] = {'channels':[16,30,24,25,18,28,26,23]}
            probe[6] = {'channels':[27,39,47,46,29,42,45,31]}
            probe[7] = {'channels':[44,33,35,37,43,36,40,41]}
        else:
            probe = {}
            probe[0] = {'channels':[31,18,27,29,26,21,23,22]}
            probe[1] = {'channels':[25,37,16,17,20,35,33,19]}
            probe[2] = {'channels':[32,46,39,38,34,44,41,36]}
            probe[3] = {'channels':[40,24,42,30,47,28,45,43]}
            probe[4] = {'channels':[6,54,0,52,2,49,53,51]}
            probe[5] = {'channels':[48,62,56,57,50,60,58,55]}
            probe[6] = {'channels':[59,7,15,14,61,10,13,63]}
            probe[7] = {'channels':[12,1,3,5,11,4,8,9]}

        geo = [(190,0),(209,-30),(192,-60),(207,-90),(194,-120),(205,-150),(196,-180),(200,-210)]
        
        for nb, shank in probe.iteritems():
            #make graph
            shank_ch = shank['channels']
            ch_map = []
            for i in range(0,len(shank_ch)-1):
                ch_map.append(((shank_ch[i]+ch_offset),(shank_ch[i+1]+ch_offset)))
                if i<(len(shank_ch)-2):
                    ch_map.append(((shank_ch[i]+ch_offset),(shank_ch[i+2]+ch_offset)))
            probe[nb]['graph'] = ch_map
            #make geometry
            probe[nb]['geometry'] = {}
            for i, ch in enumerate(probe[nb]['channels']):
                tmp = (geo[i][0]+2200*nb,geo[i][1])
                probe[nb]['geometry'][ch+ch_offset] = tmp
            #add offset to channels
            new_shank_ch = [x+ch_offset for x in shank['channels']]
            probe[nb]['channels'] = new_shank_ch
        return probe
