...
Author : Mikkel Vestergaard based on code from Chris Rodgers
...
class KlustaKwikIO(BaseIO):
    ...
    def __init__(self, filename, sampling_rate=39302.):
        ...
        self.split_table = self.read_split_file(self.filename)
    ...
    def _load_spike_times(self, fetfilename):
        ...
        names = ['fet%d' % n for n in xrange(nbFeatures-1)]
        ...
        # get features
        features = np.array([data['fet%d' % n] for n in xrange(nbFeatures-1)])
    ...
    def read_split_file(self, dirname):
        split_table = []
        f = file(os.path.join(dirname, 'split_file.txt'), 'r')
        # Number of clustering features is integer on first line
        ...
        for line in lines:
            ...
            split_table.append([file_nb, int(length)])
        return split_table
